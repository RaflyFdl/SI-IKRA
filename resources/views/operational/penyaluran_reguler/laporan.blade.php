...
<head>
    ...
    <title>Laporan Pengeluaran Reguler - IKRA</title>
...
<body class="bg-slate-100 font-sans text-slate-800">
    ...
    <div class="flex min-h-screen">
        <div class="w-64 bg-slate-900 text-white p-6 space-y-6 flex flex-col justify-between hidden md:flex shrink-0">
            <div class="space-y-6">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="{{ route('operational.penyaluran-reguler.index') }}" class="flex items-center space-x-3 bg-emerald-600 text-white font-medium p-3 rounded-lg transition">
        ...
        <div class="flex-1 p-10 max-w-6xl mx-auto">
            <div class="mb-6">
                <a href="{{ route('operational.penyaluran-reguler.index') }}" class="text-xs font-bold text-emerald-600 hover:underline">
                    <i class="fa-solid fa-arrow-left mr-1"></i> Kembali ke Rencana Penyaluran
                </a>
                <h1 class="text-2xl font-black text-slate-900 mt-2">Form Multi-Nota Rincian Penggunaan Dana</h1>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 mb-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <span class="text-slate-400 block font-bold uppercase tracking-wider text-[10px]">Nama Program Kerja</span>
                    <span class="font-bold text-slate-800 text-sm block mt-0.5">{{ $program->nama_program }}</span>
                </div>
                <div>
                    <span class="text-slate-400 block font-bold uppercase tracking-wider text-[10px]">Target Penerima Manfaat</span>
                    <span class="font-bold text-slate-700 text-sm block mt-0.5">{{ $program->penerima_manfaat }}</span>
                </div>
                <div class="bg-emerald-50 border border-emerald-100 p-3 rounded-xl">
                    <span class="text-emerald-800 block font-bold uppercase tracking-wider text-[10px]">Dana Dicairkan Pembina</span>
                    <span id="danaDicairkanBadge" data-nominal="{{ $program->nominal_diajukan }}" class="font-black text-emerald-700 text-base block mt-0.5">
                        Rp {{ number_format($program->nominal_diajukan, 0, ',', '.') }}
                    </span>
                </div>
            </div>

            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                <form action="{{ route('operational.penyaluran-reguler.laporan.store', $program->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                        ...
                        <table class="w-full text-left border-collapse text-xs" id="notaTable">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider">
                                    <th class="py-3 px-4 w-[18%]">Tanggal Nota</th>
                                    <th class="py-3 px-4 w-[37%]">Uraian / Keterangan Belanja</th>
                                    <th class="py-3 px-4 w-[20%]">Nominal (Rp)</th>
                                    <th class="py-3 px-4 w-[20%]">Bukti Nota</th>
                            ...
                            <tbody class="divide-y divide-slate-100 font-medium">
                                <tr>
                                    ...
                                    <td class="py-3 px-2">
                                        <input type="text" name="nota[0][uraian]" class="w-full p-2 border border-slate-200 rounded-lg text-xs focus:outline-none focus:border-emerald-500" placeholder="Contoh: Beli Beras 50 Karung" required>
                                    </td>
                                    <td class="py-3 px-2">
                    ...
                    <div class="flex justify-between items-center border-t border-slate-100 pt-4">
                        <a href="{{ route('operational.penyaluran-reguler.index') }}" class="bg-slate-200 hover:bg-slate-300 text-slate-700 text-xs font-bold px-5 py-2.5 rounded-xl transition">
                            Batal
                        </a>
